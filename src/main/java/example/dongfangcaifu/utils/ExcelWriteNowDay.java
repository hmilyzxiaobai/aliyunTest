...
public class ExcelWriteNowDay {

    // 写入 时间 股票名字 股票代码 涨跌情况
    ...
    public static void writeExcel(List<String> headers,
        ...
        // 创建工作表
        Sheet sheet = workbook.createSheet(pathDf);
        ...
        // 指定输出文件夹和文件名
        String outputFolder = "D:\\东方财富分析日报\\涨跌停数据日报\\"+pathDf; // 替换为你想要的文件夹路径
        folderCreatorLegacy(outputFolder);
        String fileName = pathDf+"--"+convert(type)+".xlsx";
    ...
    private static String convert(Integer type){
        switch (type){
            case 1:return "沪A涨停";
            case 2:return "深A涨停";
            case 3:return "沪A跌停";
