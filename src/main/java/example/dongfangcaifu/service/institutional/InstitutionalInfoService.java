...
public class InstitutionalInfoService extends ServiceImpl<InstitutionalInfoMapper, InstitutionalInfoEntity> {
    ...
    // 保存机构信息
    public void saveInstitutionalInfo(Integer page,Integer size){
        saveEntity(page,size);
        log.info("开始保存数据，保存条数为{}，当前页码为{}",saveList.size(),page);
        this.saveBatch(saveList);
    }
    private void saveEntity(int page,int size){
        try {
            ...
            for(int i=0;i<objects.size();i++){
                ...
                institutionalPurchaseService.saveInstitutional(1,30,one.get("OPERATEDEPT_CODE").toString());
            }
            if (page>=pageNow){
                return;
            }else {
