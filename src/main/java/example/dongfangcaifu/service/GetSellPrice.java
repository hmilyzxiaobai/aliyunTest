...
public class GetSellPrice {
    ...
    public String getNewPrice(String code){
        code=code.replace(".0","");
        CompanyInfoEntity one = companyInfoService.getOne(Wrappers.<CompanyInfoEntity>lambdaQuery().eq(CompanyInfoEntity::getCompanyCode, code).last("limit 1"));
        log.info(code);
        String appearMarket = one.getAppearMarket();
        String type= changeEnums(appearMarket);
...
//                return "";
//            }
            return jsonObject.get("f44").toString();
            // 获取最高值
        }catch (Exception e){
            //  e.printStackTrace();
        }
